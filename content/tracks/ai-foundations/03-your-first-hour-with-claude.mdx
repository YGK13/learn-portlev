...
<AnswerBlock question="How does a non-technical executive actually start using AI in one hour?">
Spend one focused hour on real work, not a course. Set up a paid Claude account (about 10 minutes). Bring one genuine task you already owe this week, a memo, a summary, a first draft. Brief the AI in four parts: who you want it to be, the context it needs, the task and the format you want back. Then push on its first answer two or three times instead of accepting it, because the value is in the iteration and your judgment is what directs it. Before you close the tab, save the prompt that worked into a Project so the second use is faster. That hour, done on your own desk, is the whole gap between you and a peer who already uses AI fluently.
</AnswerBlock>
