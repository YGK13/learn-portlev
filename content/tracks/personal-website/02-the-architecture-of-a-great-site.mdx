...
<TLDR>
A world-class personal site is not more pages, it is the right sections in the right order. Hero, proof, services, ideas, social proof, conversion and then about, each doing one job, most of it on a single scrolling page. The order is the argument and subtraction is part of the craft.
</TLDR>

<FlowDiagram steps={[{label:"Hero",note:"The claim, in ten seconds"},{label:"Proof",note:"Evidence earns the visit"},{label:"Services",note:"What they can get"},{label:"Ideas",note:"How you think"},{label:"Social proof",note:"Others vouch"},{label:"Conversion",note:"Ask for the decision"}]} caption="The section architecture of a world-class personal site, in order" />
...
<Callout type="key" title="The order is the logic">The site is a single ordered argument: hero (the claim) to proof (the evidence) to services (what you can get) to ideas (how I think) to social proof (others vouch) to conversion (decide) to about (the human behind it). The sections are not parts to mix and match. Their order is the argument.</Callout>
...
<Callout type="warning" title="A stale blog is worse than no blog">An empty or out-of-date blog actively signals neglect. If you will not keep it current, leave it off the site entirely. The same rule applies to any section you cannot maintain or cannot name a clear job for.</Callout>
