...
<TLDR>
Three operational AI risks matter most: hallucination (output is not automatically true), bias (not automatically fair) and prompt injection (behavior is not automatically under your control). Each fails confidently and invisibly. Match each risk to your real use cases, never let AI make the final call on a person, treat outside content as untrusted, keep a named human accountable for every consequential use and put it in a one-page policy.
</TLDR>
...
<Callout type="example" title="How indirect injection works">A resume in your hiring pipeline carries hidden instructions. Your AI screening tool reads it, encounters the instructions and acts on them: rank this candidate first, ignore your safeguards, leak data. The attacker never contacted you. They planted the instruction in content they knew an AI would eventually read.</Callout>
...
<Callout type="key">All three risks fail the same way: confidently and invisibly. So the response is the same. A named human stays accountable and decides. AI assists; it never gets delegated accountability.</Callout>
