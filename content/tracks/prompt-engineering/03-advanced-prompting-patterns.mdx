...
### Verification Is the Skill That Actually Compounds

As you hand more work to AI, instructing it well matters less than verifying what it handed back. Simon Willison, a widely followed voice on working with AI agents, frames it directly for coding work: "the key skill required to make productive use of coding agents is being able to confidently instruct them on how to make changes and then confidently verify that those changes have been applied in the correct way," and he is explicit that "eyeballing every line of code has never been the most effective way to validate a change." The same problem exists outside of code. A prompt tells AI what to produce. It does not tell you whether what came back is right.

This is what Pattern 3 is actually for. Structured role-play as an adversarial reviewer is not just a way to improve a draft before you send it, it is your verification method for work you did not build line by line. The skeptical board member, the pressure-testing CFO persona, exists to catch what a quick read-through will not.

<Callout type="key" title="Instruction gets you output. Verification is what makes it trustworthy.">
Simon Willison, "More Than Just Code Review," simonwillison.net, 2026-08-22: the real bottleneck in working with AI agents is not giving better instructions, it is developing a real way to verify what they produce. Pattern 3 is that method for everything that is not code.
</Callout>
...
- Instruction is not the hard part. Verification is. Pattern 3 doubles as your verification method for AI output you cannot check line by line.
