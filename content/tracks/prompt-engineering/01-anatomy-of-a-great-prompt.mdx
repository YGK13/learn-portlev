...
The professionals on the right side of the [AI Wage Gap](/learn/ai-foundations/what-is-the-ai-wage-gap) treat AI differently. They write **specifications**: precise instructions that tell the AI exactly who it is, exactly what they need and exactly what the output should look like. That shift in mindset produces a completely different tier of output, and it is what this track is about.

<TLDR>
A prompt is not a question, it is a specification. Every effective prompt has three components: Role (who the AI is), Task (exactly what it must produce) and Context (what it needs to know). Get those three right and AI output jumps a full tier in quality.
</TLDR>

<Callout type="key" title="The core mindset shift">Stop asking. Start specifying. A question gets a generic answer. A specification gets the deliverable you actually wanted.</Callout>
...
<KnowledgeCheck
  ...
  questions={[
    {
      question: "What separates a prompt from a search query?",
      options: [
        "A prompt is longer and more polite",
        "A prompt specifies role, task and context instead of asking a question",
        "A prompt always includes a word count"
      ],
      answer: 1,
      explain: "Length and politeness change nothing. Specification does: who the AI is, exactly what it must produce and what it needs to know."
    },
    {
      question: "Which line carries the most leverage in any prompt?",
      options: [
        "The role assignment",
      ...
      answer: 0,
      explain: "Role assignment primes the AI's entire register. A senior consultant persona writes a fundamentally different document than the default assistant."
    },
    {
      question: "You keep heavily editing AI output. Per this lesson, the most likely cause is:",
      options: [
        "The model is not powerful enough",
        "You did not provide enough context and constraints",
        "You asked during peak hours"
      ],
      answer: 1,
      explain: "Every hour spent editing AI output is an hour lost to insufficient context. Feed it the background, the format and the anti-patterns and the editing collapses."
    }
  ]}
/>
...
<Callout type="warning" title="If you do not name it, the AI will do it">Every hedge, every "certainly!", every invented statistic shows up because you did not explicitly forbid it. Anti-patterns are not optional polish, they are the difference between output you ship and output you rewrite.</Callout>

<TryIt id="anatomy-tryit" title="Run the three-part structure on real work" prompt={`You are a [specific expert] who [specific experience and specialty].

I need [deliverable type] that [specific function].
It should be [length/format].
It must include: [component 1], [component 2], [component 3].
It should not: [anti-pattern 1], [anti-pattern 2].
The tone is [X]. The audience is [Y] who [what they care about].

Context you need:
[Paste the document, data or background here]`}>
...
<ActionChecklist
  ...
  items={[
    "Write your five role descriptions (Step 1)",
    "Fill in the output specification template on a real task (Step 2)",
    "Test the three-part structure on one real deliverable (Step 3)",
    "Add a default anti-pattern list to your standard prompt (Step 4)"
  ]}
/>
