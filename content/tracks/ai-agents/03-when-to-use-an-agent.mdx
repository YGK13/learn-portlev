...
<Callout type="key" title="A frontier variant worth knowing">
The model above sets the autonomy dial in advance, by task and stakes. A newer variant lets a dedicated facilitator agent make that call in real time instead: not the agent doing the work, but a separate agent whose only job is deciding when to loop a human in, based on whether the moment needs approval, needs expertise the system lacks, needs a second perspective or is a decision worth a human's judgment. Researcher Ethan Mollick calls this a "Twilight Factory," distinct from a fully unreviewed "Software Factory" where no human writes or reviews the output at all. For most leaders reading this, the static model above is still the right starting point: decide the tiers before you build. The dynamic, agent-decided version is where the frontier is heading next.
</Callout>
