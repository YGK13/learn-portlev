...
<TLDR>
A tool is a specific action an AI is allowed to take, and the list of an AI's tools is both its reach and its risk. The AI never runs a tool itself: it asks a surrounding system to act and reads the result back, which is where every permission check lives. MCP is a new connection standard, USB-C for AI, that replaces custom one-off integrations with reusable connectors and means less lock-in. Every connection adds capability and risk together, so grant the least access that does the job.
</TLDR>
...
<Callout type="key" title="The one question that tells you everything">
If you want to know what an AI agent can do, and what it could do wrong, ask for the list of its tools. That list is the agent's reach and its risk surface in the same breath.
</Callout>
...
<Callout type="warning" title="Connection cuts both ways">
Every capability you grant a connected AI is also a way it can do damage. The email tool that drafts for you can send the wrong thing. The database tool that updates records can corrupt them. Treat each new connection as a new risk, not just a new convenience.
</Callout>

<Pullquote author="Kim Pecina" role="HR and benefits leader" source="AI Wage Gap Podcast">
